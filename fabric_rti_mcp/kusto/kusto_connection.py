...
class KustoConnection:
    ...
    def __init__(self, cluster_uri: str, useOBO: Optional[bool], user_token: Optional[str], default_database: Optional[str] = None):

        cluster_uri = sanitize_uri(cluster_uri)
        kcsb = KustoConnectionStringBuilder.with_azure_token_credential(
            connection_string=cluster_uri,
            credential_from_login_endpoint=lambda login_endpoint: self._get_credential(login_endpoint, useOBO, user_token),
        )
    ...
    def _get_credential(self, login_endpoint: str, useOBO: Optional[bool], user_token: Optional[str]) -> ChainedTokenCredential:
        if useOBO and user_token:
            ...
            return ChainedTokenCredential(
                OnBehalfOfCredential(
                    tenant_id=os.environ.get('TENANT_ID',''), # ID of the service principal's tenant. Also called its "directory" ID.
                    user_assertion=user_token, # This is the user token passed in from front-end.
                    client_id=os.environ.get('CLIENT_ID',''), # The App Registration ID of the middle-tier service. That is this service, NOT the user.
    ...
    def _get_certificate_from_keyvault(self,
    keyvault_url: str,
